#!/usr/bin/env python3
"""
Real Items Scraper - CORRECT METHOD
- Select organization WITHOUT icon
- Go to Stores tab  
- Scrape each store
- Page size 100
...
def main():
    log("="*70)
    log("REAL ITEMS SCRAPER - Correct Method")
    log("="*70)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Use the store dropdown that's ALREADY on the page (NOT language dropdown)
            log("\nStep 3: Getting all stores from the page's store dropdown...")

            # Click the STORE selector dropdown (skip the first selectors which are language/org)
            # The store selector is in the main content area
            try:
                # Try clicking the selector that contains a store name
                page.click('text=/HOKKIEN MEE|CHICKEN RICE|HUMFULL|Mee Pok/i >> xpath=ancestor::div[contains(@class, "ant-select")]', timeout=5000)
            except:
                # Fallback: click the 3rd or 4th ant-select-selector on the page
                selectors = page.query_selector_all(".ant-select-selector")
                if len(selectors) >= 3:
                    selectors[2].click()  # Skip language and org dropdowns
                else:
            ...
            for scroll_attempt in range(40):
                ...
                try:
                    page.evaluate("""
                        ...
                        if (holder) holder.scrollTop += 500;
                    """)
            ...
            for idx, store_name in enumerate(stores, 1):
                ...
                try:
                    ...
                    # Click store dropdown (skip language/org dropdowns)
                    try:
                        # Try clicking a selector with store text
                        page.click('text=/HOKKIEN MEE|CHICKEN RICE|HUMFULL|Mee Pok|Western|Drinks/i >> xpath=ancestor::div[contains(@class, "ant-select")]', timeout=3000)
                    except:
                        # Fallback: click 3rd selector
                        selectors = page.query_selector_all(".ant-select-selector")
                        if len(selectors) >= 3:
                            selectors[2].click()
                        else:
