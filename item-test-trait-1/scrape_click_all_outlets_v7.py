...
Version 7: Only click outlets belonging to CURRENT brand (46 total, not 94!)
"""
...
# Log file path
LOG_FILE = "item-test-trait-1/scrape_click_outlets_v7.log"
...
with sync_playwright() as p:
    ...
    log("="*70)
    log("STARTING OUTLET CLICKING TEST V7")
    log("="*70)
    ...
    if not ensure_group_view(page):
        ...
        page.screenshot(path="item-test-trait-1/failed_group_view_v7.png")
        browser.close()
        sys.exit(1)

    # Take screenshot of starting state
    page.screenshot(path="item-test-trait-1/step1_group_view_v7.png")
    log("Screenshot saved: step1_group_view_v7.png\n")
    ...
    for idx, brand_name in enumerate(BRANDS, 1):
        ...
        try:
            ...
            # Get all child store nodes that belong to THIS brand only
            outlets_clicked_for_brand = 0

            # Find all visible outlet elements
            ...
            for store_elem in all_store_elements:
                try:
                    ...
                    # IMPORTANT: Only click if:
                    # 1. Has @ symbol (is an outlet, not a brand)
                    # 2. NOT in the BRANDS list
                    # 3. NOT already clicked before
                    # 4. Is visible
                    # 5. Belongs to current brand (check if it starts with brand name or contains brand keywords)

                    if store_title and '@' in store_title and store_title not in BRANDS:
                        # Skip if already clicked
                        if store_title in clicked_outlet_names:
                            continue

                        # Check if it's visible
                        if store_elem.is_visible():
                            # Determine if this outlet belongs to current brand
                            # For brands like "Le Le Mee Pok", check if outlet starts with it
                            # For "OK Chicken Rice", check for "OK CHICKEN RICE"
                            # For "AH Huat Hokkien Mee", check for "AH HUAT HOKKIEN"

                            belongs_to_brand = False

                            if brand_name == "Le Le Mee Pok" and "Le Le Mee Pok" in store_title:
                                belongs_to_brand = True
                            elif brand_name == "JKT Western" and "JKT Western" in store_title:
                                belongs_to_brand = True
                            elif brand_name == "Drinks Stall" and ("Drinks Stall" in store_title or "Toa Payoh Drinks" in store_title):
                                belongs_to_brand = True
                            elif brand_name == "HUMFULL" and "HUMFULL" in store_title:
                                belongs_to_brand = True
                            elif brand_name == "OK Chicken Rice" and "OK CHICKEN RICE" in store_title:
                                belongs_to_brand = True
                            elif brand_name == "AH Huat Hokkien Mee" and "AH HUAT HOKKIEN" in store_title:
                                belongs_to_brand = True
    ...
    screenshot_path = "item-test-trait-1/all_outlets_clicked_v7.png"
